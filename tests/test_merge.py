...
from koza.model.graph_operations import (
    MergeConfig, FileSpec, KGXFormat, KGXFileType, DatabaseStats,
    JoinResult, NormalizeResult, PruneResult, OperationSummary
)
...
def sample_file_specs():
    ...
    edge_specs = [
        FileSpec(path=Path("edges1.tsv"), format=KGXFormat.TSV, file_type=KGXFileType.EDGES)
    ]
    mapping_specs = [
        FileSpec(path=Path("mappings.sssom.tsv"), format=KGXFormat.TSV, file_type=None)
    ]
...
class TestMergeOperationOrchestration:
    ...
    def test_full_pipeline_all_operations(
        self, mock_graph_db, mock_prune, mock_normalize, mock_join,
        sample_file_specs, mock_join_result, mock_normalize_result, mock_prune_result
    ):
    ...
    def test_skip_normalize_only_join_and_prune(
        self, mock_graph_db, mock_prune, mock_normalize, mock_join,
        sample_file_specs, mock_join_result, mock_prune_result
    ):
    ...
    def test_skip_prune_only_join_and_normalize(
        self, mock_graph_db, mock_prune, mock_normalize, mock_join,
        sample_file_specs, mock_join_result, mock_normalize_result
    ):
    ...
    def test_skip_both_normalize_and_prune_only_join(
        self, mock_graph_db, mock_prune, mock_normalize, mock_join,
        sample_file_specs, mock_join_result
    ):
...
class TestMergeOperationErrorHandling:
    ...
    @patch('koza.graph_operations.merge.join_graphs')
    @patch('koza.graph_operations.merge.GraphDatabase')
    def test_join_failure_stops_pipeline(
        self, mock_graph_db, mock_join, sample_file_specs
    ):
...
class TestMergeHelperFunctions:
    ...
    def test_prepare_merge_config_from_paths(
        self, mock_prepare_mappings, mock_prepare_files, sample_file_specs
    ):
