...
def get_logger(verbose: bool = False, filename=None):
    # os.environ["LOGURU_FORMAT"] = "[<green>{time:YYYY-MM-DD HH:mm:ss}</green>] [<level>{level: <8}</level>] [<cyan>{name}</cyan>] [<level>{message}</level>]"
    logger = loguru.logger
    logger.remove()
    logger.add(
        sink=f"logs/{filename}.log" if filename else sys.stderr,
        level="INFO" if (verbose is None) else "DEBUG" if (verbose == True) else "WARNING",
        format="<green>{time:YYYY-MM-DD HH:mm:ss}</green> | <level>{level: <8}</level> | <cyan>{name: <16}</cyan> | <level>{message}</level>",
        colorize=True,
    )
    return logger
