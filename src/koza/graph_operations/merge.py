...
from koza.model.graph_operations import (
    MergeConfig, MergeResult, JoinConfig, NormalizeConfig, PruneConfig,
...
def merge_graphs(config: MergeConfig) -> MergeResult:
    ...
    try:
        if not config.quiet:
            ...
            print(f"📊 Pipeline: join → {'normalize → ' if not config.skip_normalize else ''}{'prune' if not config.skip_prune else ''}")
            if using_temp_db:
        ...
        if not config.skip_normalize:
            ...
            if not normalize_result.success:
                ...
                if not config.quiet:
                    ...
                    print(f"✓ Normalize completed: {mappings_count} mapping files → {normalized_count:,} edge references normalized")
        else:
